...
def env_subst_file(sourcefile, hashref_path, destfile, input_path):
    ...
    sections=["CHANGEPARAM","SUBSYSTEMPARAM","COPYCOMP","DISABLECOMP"]
...
def docker_compose_wrapper(input_path, yaml_file_path, HOMEDIR, patchsetnum, cleanup=None):
    try:
...
def gitcloneexe(HOMEDIR):
    ...
    if not os.path.exists(repo_dir):
        ...
        clone_url="git@github.com:Dymensions/dy-cicd-exe.git"
        os.system("git clone --depth 1 -b dy-cicd-src-inhouse_STAGE " + clone_url + " " + repo_dir)
        print("git clone --depth 1 " + clone_url + " " + repo_dir)
        os.chdir(repo_dir)
        #print ("Pull complete")
    else:
...
def main():
    ...
    if args.function_name == "convert_json":
        ...
        docker_compose_wrapper(input_path, yaml_file_path, HOMEDIR, patchsetnum, args.cleanup)
